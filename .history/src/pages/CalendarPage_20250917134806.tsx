...
{/* Priority Filters - only show when expanded */}
        ...
        {!sidebarCollapsed && (
          <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex-1 overflow-y-auto">
            <h3 className="text-sm font-medium text-gray-900 dark:text-white mb-3">SUMMARY</h3>
            
            {/* Ongoing Events */}
            <div className="mb-4">
              <h4 className="text-xs font-medium text-gray-600 dark:text-gray-400 mb-2 uppercase">Ongoing Events</h4>
              <div className="space-y-1 max-h-32 overflow-y-auto">
                {(() => {
                  ...
                  const ongoingEvents = getVisibleTasks().filter(task => {
                  ...
                  return ongoingEvents.map((task) => (
                    <div key={task.id} className="flex items-center justify-between group p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md">
                      ...
                      <button
                        ...
                        className="opacity-0 group-hover:opacity-100 ml-2 text-red-500 hover:text-red-600 transition-all"
                        title="Delete event"
                      >
            ...
            {/* Past Events */}
            <div>
              <h4 className="text-xs font-medium text-gray-600 dark:text-gray-400 mb-2 uppercase">Past Events</h4>
              <div className="space-y-1 max-h-40 overflow-y-auto">
                {(() => {
                  ...
                  const pastEvents = getVisibleTasks().filter(task => {
                    // Events from previous dates
                    if (task.date < todayString) return true;
                    
                    // Events from today that have ended
                    if (task.date === todayString) {
                      if (task.allDay) return false; // All day events are never "past" on the same day
                      if (!task.startTime || !task.endTime) return false;
                    ...
                    <div key={task.id} className="flex items-center justify-between group p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-gray-500 dark:text-gray-400">
                          ...
                          {task.startTime && !task.allDay && ` • ${task.startTime}`}
                        </div>
                      </div>
                      <button
                        onClick={() => setDeleteConfirm(task.id)}
